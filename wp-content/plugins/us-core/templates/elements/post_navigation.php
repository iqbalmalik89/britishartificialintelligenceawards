...
// Disable Lazy Load for images with Sided layout
$attachment_attr = ( $layout == 'sided' ) ? array( 'class' => 'not-lazy' ) : array();
...
foreach ( $prevnext as $key => $item ) {
	...
	if ( ! empty( $prevnext[ $key ] ) ) {
		...
		if ( $tnail_id ) {
			$image = wp_get_attachment_image( $tnail_id, 'thumbnail', FALSE, $attachment_attr );
		}
