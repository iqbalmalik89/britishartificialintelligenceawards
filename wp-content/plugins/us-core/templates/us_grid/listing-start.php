...
if ( ! empty( $items_gap ) AND $type != 'metro' ) {
	if ( $columns != 1 ) {
...
// Calculate items Aspect Ratio via CSS
if ( $items_have_ratio ) {

	// If Aspect Ratio is default in Grid settings, use Grid Layout settings
	if ( $items_ratio == 'default' ) {
		$items_ratio = us_arr_path( $grid_layout_settings, 'default.options.ratio' );
		$items_ratio_width = us_arr_path( $grid_layout_settings, 'default.options.ratio_width' );
		$items_ratio_height = us_arr_path( $grid_layout_settings, 'default.options.ratio_height' );
	}
	...
	if ( $items_ratio == '4x3' ) {
		$grid_elm_ratio_w = 4;
		$grid_elm_ratio_h = 3;
	} elseif ( $items_ratio == '3x2' ) {
		$grid_elm_ratio_w = 3;
		$grid_elm_ratio_h = 2;
	} elseif ( $items_ratio == '2x3' ) {
		$grid_elm_ratio_w = 2;
		$grid_elm_ratio_h = 3;
	} elseif ( $items_ratio == '3x4' ) {
		$grid_elm_ratio_w = 3;
		$grid_elm_ratio_h = 4;
	} elseif ( $items_ratio == '16x9' ) {
		$grid_elm_ratio_w = 16;
		$grid_elm_ratio_h = 9;
	} elseif ( $items_ratio == 'custom' ) {
		$grid_elm_ratio_w = floatval( str_replace( ',', '.', preg_replace( '/^[^\d.,]+$/', '', $items_ratio_width ) ) );
		if ( $grid_elm_ratio_w <= 0 ) {
			$grid_elm_ratio_w = 1;
		}
		$grid_elm_ratio_h = floatval( str_replace( ',', '.', preg_replace( '/^[^\d.,]+$/', '', $items_ratio_height ) ) );
...
foreach ( $grid_layout_settings['data'] as $elm_id => $elm ) {
	...
	if ( isset( $elm['hover'] ) AND $elm['hover'] ) {
		...
		if ( isset( $elm['transform_origin_X'] ) AND isset( $elm['transform_origin_Y'] ) AND $elm['transform_origin_X'] != '50%' AND $elm['transform_origin_Y'] != '50%' ) {
			$css_layout .= 'transform-origin: ' . $elm['transform_origin_X'] . ' ' . $elm['transform_origin_Y'] . ';';
		}
