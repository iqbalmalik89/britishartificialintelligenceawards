...
class us_migration_5_0 extends US_Migration_Translator {
	...
	public function migration_completed_message() {
		...
		$output .= '<p><strong>Update to ' . US_THEMENAME . ' ' . US_THEMEVERSION . ' is completed</strong>. Now check your website. If you notice some issues, <a href="https://help.us-themes.com/impreza/tickets/" target="_blank">go to the support</a>.</p>';
		$output .= '</div>';
	...
	public function translate_theme_options( &$options ) {
		...
		if ( isset( $options['optimize_assets'] ) AND $options['optimize_assets'] == 1 AND is_array( $options['assets'] ) ) {
			$options['assets'] = array_merge( $options['assets'], array( 'grid' ) );
		}
