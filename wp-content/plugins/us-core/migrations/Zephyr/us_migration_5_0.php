...
class us_migration_5_0 extends US_Migration_Translator {
	...
	public function migration_completed_message() {
		...
		$output .= '<p><strong>Update to ' . US_THEMENAME . ' ' . US_THEMEVERSION . ' is completed</strong>. Now check your website. If you notice some issues, <a href="https://help.us-themes.com/' . strtolower( US_THEMENAME ) . '/tickets/" target="_blank">go to the support</a>.</p>';
		$output .= '</div>';
	...
	public function translate_theme_options( &$options ) {
		...
		if ( $templates_config = $this->get_grid_templates() AND isset( $templates_config[$blog_layout_name] ) ) {
			...
			// Create the grid layout post
			$layout_id = $this->add_grid_layout( 'layout_blog_home', $layout['title'] . ' - ' . __( 'Blog Home Page', 'us' ), $layout );
		...
		if ( $templates_config = $this->get_grid_templates() AND isset( $templates_config[$blog_layout_name] ) ) {
			...
			// Create the grid layout post
			$layout_id = $this->add_grid_layout( 'layout_archive', $layout['title'] . ' - ' . __( 'Archive Pages', 'us' ), $layout );
		...
		if ( $templates_config = $this->get_grid_templates() AND isset( $templates_config[$blog_layout_name] ) ) {
			...
			// Create the grid layout post
			$layout_id = $this->add_grid_layout( 'layout_search_results', $layout['title'] . ' - ' . __( 'Search Results Page', 'us' ), $layout );
