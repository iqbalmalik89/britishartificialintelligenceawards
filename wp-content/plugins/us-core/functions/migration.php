...
class US_Migration {
	...
	protected function __construct() {
		...
		if ( empty( $this->translators ) ) {
			...
			if ( $db_version != self::get_latest_version_migration() ) {
	...
	public function get_db_version() {
		...
		$result = get_option( 'us_db_version' ) ? get_option( 'us_db_version' ) : self::get_latest_version_migration();
	...
	public function set_db_version( $version = NULL ) {
		if ( $version === NULL ) {
			$version = self::get_latest_version_migration();
		}
	...
	public function migration_completed_message() {
		if ( $this->migration_completed_message != '' ) {
			...
			$output .= 'Now check your website. If you notice some issues, <a href="https://help.us-themes.com/' . strtolower( US_THEMENAME ) . '/tickets/" target="_blank">go to the support</a>.</h3>';
		}
	...
	public function perform_migration_step( $num_posts = 50 ) {
		...
		$query_post_args = array(
			'posts_per_page' => - 1,
			'post_type' => us_array_merge( $predefined_posts_types, $public_cpt ),
			'post_status' => 'any',
			'numberposts' => - 1,
			'orderby' => 'ID',
			'order' => 'ASC',
		...
		$count_posts = 0;
		foreach ( us_array_merge( $predefined_posts_types, $public_cpt ) as $post_type ) {
		...
		unset( $predefined_posts_types, $public_cpt );
	...
	public function migrate_wpb_templates() {
		...
		if ( $wpb_js_templates = get_option( 'wpb_js_templates' ) AND is_array( $wpb_js_templates ) ) {
			foreach ( $wpb_js_templates as &$temp ) {
				if ( isset( $temp['template'] ) AND ! empty( $temp['template'] ) AND method_exists( $this->current_translator, 'translate_content' ) ) {
