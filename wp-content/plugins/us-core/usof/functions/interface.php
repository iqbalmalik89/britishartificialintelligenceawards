...
function us_theme_options_page() {
	...
	global $usof_options, $usof_body_fontsize, $usof_img_sizes, $usof_enable_portfolio, $usof_portfolio_label, $usof_sidebar_titlebar;
	usof_load_options_once();
	$usof_options = array_merge( usof_defaults(), $usof_options );
	$usof_img_sizes = us_image_sizes_select_values();
	$usof_body_fontsize = $usof_options['body_fontsize'];
	$usof_enable_portfolio = $usof_options['enable_portfolio'];
	$usof_sidebar_titlebar = $usof_options['enable_sidebar_titlebar'];
	if ( $usof_options['portfolio_rename'] ) {
		$usof_portfolio_label = wp_strip_all_tags( $usof_options['portfolio_label_name'], TRUE );
	}
...
function usof_print_scripts() {
	...
	wp_enqueue_script( 'usof-select2', US_CORE_URI . '/usof/js/select2.min.js', array( 'jquery' ), US_CORE_VERSION, TRUE );
