...
! function( $ ) {
	...
	$usof.field[ 'font' ] = {
		...
		_init: function() {
			// Google Fonts
			this.$googleFonts = '';
			this.$googleGroup = this.$select.find( 'optgroup.google' );
			this.currentGoogleFont = this.$googleGroup.data( 'current-font' );

			for ( var i = 0 in this.fonts ) {
				if ( ! this.fonts.hasOwnProperty( i ) ) {
					continue;
				}

				var selected = '';
				if ( this.currentGoogleFont == i ) {
					selected = ' selected';
				}

				var $option = '<option value="' + i + '"' + selected + '>' + i + '</option>';
				this.$googleFonts += $option;
			}

			if ( this.$googleFonts ) {
				this.$googleGroup.html( this.$googleFonts );
			}
	...
	$usof.Group.prototype = {
		...
		init: function( elm, options ) {
			...
			this.$btnDuplicateGroup.on( 'click', this.duplicateGroup.bind( this ) );

			this.$btnDelGroup.on( 'click', function( event ) { // TODO: check whether adding event for each button is leaner
				event.stopPropagation();
			...
			if ( this.isAccordion ) {
				...
				this.$sectionTitles.on( 'click', function( event ) {
					this.$sections = this.$field.find( '.usof-form-group-item' );
			...
			if ( this.isSortable ) {
				...
				this.$dragControls.on( 'mousedown', this._dragStart.bind( this ) );
