(function($){"use strict";$us.WGrid=function(container,options){this.init(container,options)};$us.WGrid.prototype={init:function(container,options){this.$container=$(container);this.$filters=this.$container.find('.g-filters-item');this.$list=this.$container.find('.w-grid-list');this.$items=this.$container.find('.w-grid-item');this.$pagination=this.$container.find('.pagination');this.$loadmore=this.$container.find('.g-loadmore');this.$preloader=this.$container.find('.w-grid-preloader');this.curFilterTaxonomy='';this.paginationType=this.$pagination.length?'regular':(this.$loadmore.length?'ajax':'none');this.filterTaxonomyName=this.$list.data('filter_taxonomy_name')?this.$list.data('filter_taxonomy_name'):'category';this.loading=!1;if(this.$container.data('gridInit')==1){return}
