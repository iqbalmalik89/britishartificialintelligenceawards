...
function us_core_admin_notice() {
	$output = '<div class="us-core-notice"><h2>';
	$output .= sprintf(
	...
	$output .= ' <a href="https://help.us-themes.com/' . strtolower( US_THEMENAME ) . '/us-core/" target="_blank" rel="noopener">' . __( 'Read more about it', 'us' ) . '</a>';
	$output .= '</h2></div>';
	$output .= '<style>
		.us-core-notice {
			padding: 20px;
...
function us_add_addons_page() {
	...
	add_submenu_page( $parent_slug, __( 'Addons', 'us' ), __( 'Addons', 'us' ), 'manage_options', 'us-addons', 'us_addons_page', 11 );
}
		...
		<div class="us-addons-list"><?php
			foreach ( $plugins as $plugin ) {
				...
				if ( isset( $activated_plugins[ $plugin['file_path'] ] ) ) {
					...
					if ( isset( $plugin['premium'] ) AND empty( $plugin['source'] ) AND ! get_option( 'us_license_activated', 0 ) ) {
				...
				<div class="us-addon<?php echo esc_attr( $classes ); ?>">
					...
					<div class="us-addon-control">
						...
						if ( isset( $plugin['premium'] ) AND ! isset( $plugin['source'] ) AND ! get_option( 'us_license_activated', 0 ) ) {
							?>
