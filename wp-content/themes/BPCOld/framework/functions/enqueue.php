...
function us_styles() {
	...
	wp_register_style( 'gill-sans', get_template_directory_uri() . '/fonts/stylesheet.css', array(), US_THEMEVERSION, 'all' );
	wp_enqueue_style ( 'gill-sans' );
...
function us_custom_styles() {
	...
	if ( $us_generate_css_file ) {
		...
		if ( file_exists( $styles_file ) ) {
			$styles_file_uri = $wp_upload_dir['baseurl'] . '/us-assets/' . US_THEMENAME . '-theme-options.css';
...
function us_jscripts() {
	...
	wp_register_script( 'us-custom', $us_template_directory_uri . '/js/jquery.custom.js', array( 'jquery' ), '', TRUE );
	wp_enqueue_script( 'us-custom' );
	...
	wp_register_script( 'us-mediaelement', $us_template_directory_uri . '/framework/js/mediaelement-and-player.min.js', array( 'jquery' ), '', TRUE );
